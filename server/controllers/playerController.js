import playerSchema from "../models/tenant/Player.js";
...
export const addPlayer = async (req, res) => {
    try {
        const Player = getTenantModel(req.tenantConnection, 'Player', playerSchema);
        ...
        if (req.app.get('io')) {
            ...
            req.app.get('io').to(tenantId).emit('playerAdded', player);
        }
...
export const updatePlayer = async (req, res) => {
    try {
        ...
        if (req.app.get('io')) {
            const tenantId = req.headers['x-tenant-id'];
            req.app.get('io').to(tenantId).emit('playerUpdated', player);
        }
...
export const deletePlayer = async (req, res) => {
    try {
        ...
        if (req.app.get('io')) {
            const tenantId = req.headers['x-tenant-id'];
            req.app.get('io').to(tenantId).emit('playerDeleted', req.params.id);
        }
...
export const importPlayers = async (req, res) => {
    try {
        ...
        if (req.app.get('io')) {
            const tenantId = req.headers['x-tenant-id'];
            req.app.get('io').to(tenantId).emit('playersImported', players);
        }
